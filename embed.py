...
def get_state_embedder(env):
  ...
  from envs.miniworld import construction
  if isinstance(env, construction.MiniWorldConstruction):
    return MiniWorldEmbedder
...
class TrajectoryEmbedder(Embedder, relabel.RewardLabeler):
  def __init__(self,
    ...
    self._id_embedder_mode = id_embedder_mode
    self._use_ids = True
  ...
  def set_initial_states(self, states):
    if states is not None and self._initial_states is not None:
      raise ValueError("Initial states already set.")
  ...
  def _compute_contexts(self, trajectories):
    ...
    # empty trajectories
    # first timestep of rollout case which begins with an empty trajectory
    if mask.shape[1] == 0:
      transition_hidden_states = torch.zeros(
          len(trajectories), 1, 128)
      ...
      transition_contexts = all_transition_contexts.squeeze(1)

      # grab initial state from self._initial_states set by run_episode in main.py
      assert self._initial_states is not None, "Initial states must be set."
      if isinstance(self._id_embedder, IDEmbedder):
        id_contexts = self._id_embedder(
            torch.tensor([init_state.env_id for init_state in self._initial_states]))
      elif self._id_embedder_mode == "init-pos" or self._id_embedder_mode == "curr-pos" or (
          self._id_embedder_mode == "valid-indices"
      ):
        # Initial position or current position PI
        id_contexts = self._id_embedder(
            [torch.tensor(init_state.privileged_info) for init_state in self._initial_states]
        )
      elif self._id_embedder_mode == "optimal-act":
        # Optimal action PI
        id_contexts = self._id_embedder(
            torch.tensor([
                [init_state.privileged_info]
                for init_state in self._initial_states
            ])
        )
      else:
        obs = torch.tensor([init_state.privileged_info for init_state in self._initial_states])
        id_contexts = self._id_embedder(obs)

      mask = torch.ones(transition_contexts.shape[0], 1).bool()
    ...
    if isinstance(self._id_embedder, IDEmbedder):
      if self._id_embedder_mode == "curr-env-id":
        env_ids = []
        for traj in padded_trajectories:
          env_ids.append(traj[0].state.env_id)
          for exp in traj:
            env_ids.append(exp.next_state.env_id)
        env_ids = torch.tensor(env_ids)
        id_contexts = self._id_embedder(env_ids)
        id_contexts = id_contexts.reshape(mask.shape[0], mask.shape[1] + 1, -1)
      else:
        id_contexts = self._id_embedder(
            torch.tensor([traj[0].state.env_id for traj in trajectories]))
    elif self._id_embedder_mode == "init-pos":
      # Initial position PI
      id_contexts = self._id_embedder(
          [torch.tensor(traj[0].state.privileged_info) for traj in trajectories]
      )
    elif self._id_embedder_mode == "curr-pos" or self._id_embedder_mode == "valid-indices":
      privileged_info = []
      for traj in padded_trajectories:
        privileged_info.append(torch.tensor(traj[0].state.privileged_info))
        for exp in traj:
          privileged_info.append(torch.tensor(exp.next_state.privileged_info))
      id_contexts = self._id_embedder(privileged_info)
      id_contexts = id_contexts.reshape(mask.shape[0], mask.shape[1] + 1, -1)
    elif self._id_embedder_mode == "optimal-act":
      # Optimal action PI
      privileged_info = []
      for traj in padded_trajectories:
          privileged_info.append([traj[0].state.privileged_info] +
                                 [exp.next_state.privileged_info for exp in traj])
      id_contexts = self._id_embedder(torch.tensor(privileged_info))
    else:
      obs = torch.tensor([traj[0].state.privileged_info for traj in trajectories])
      id_contexts = self._id_embedder(obs)
  ...
  def _compute_losses(
    ...
    if len(id_contexts.shape) < 3:
      # For env_id PI, we have the same id context for all time-steps.
      transition_context_loss = (
          (all_transition_contexts - id_contexts.unsqueeze(1).expand_as(
           all_transition_contexts).detach()) ** 2).sum(-1)
    else:
      # For general PI, we have different id contexts at each time-step.
      transition_context_loss = (
          (all_transition_contexts - id_contexts.detach()) ** 2).sum(-1)
    transition_context_loss = (
    ...
    if len(id_contexts.shape) < 3:
      ...
      cutoff = torch.ones(id_contexts.shape[0], id_contexts.shape[1]) * 10
      # Make sure we're only taking the id_contexts at real time-steps (not padded ones)
      id_context_loss = torch.max((id_contexts ** 2).sum(-1), cutoff)
      id_context_loss = (id_context_loss * mask).sum() / mask.sum()
    losses = {
      "transition_context_loss": transition_context_loss,
      "id_context_loss": id_context_loss,
    }
    return losses
...
class InstructionPolicyEmbedder(Embedder):
  ...
  def __init__(self, trajectory_embedder, obs_embedder, instruction_embedder,
               embed_dim, use_trajectory_embed):
    ...
    self._use_trajectory_embed = use_trajectory_embed
    if self._use_trajectory_embed:
      self._fc_layer = nn.Linear(
          obs_embedder.embed_dim + self._trajectory_embedder.embed_dim, 256)
    else:
      self._fc_layer = nn.Linear(obs_embedder.embed_dim, 256)
  ...
  def forward(self, states, hidden_state):
    ...
    if not self._use_trajectory_embed:
      hidden = F.relu(self._fc_layer(obs_embed))
      return self._final_layer(hidden), hidden_state

    # The trajectory from the last state in states contains the history so far.
    if len(obs_embed.shape) == 2:
      # Roll out case
      trajectory_embed, _ = self._trajectory_embedder(
          [state[-1].trajectory for state in states])

      # This is where we're getting id_contexts.
      # We get id_contexts at every time step of the trajectory.
      # We will use the last one.
      if len(trajectory_embed.shape) > 2:
        trajectory_embed = trajectory_embed[:, -1, :]
    else:
      # Update case where obs_embed is (batch size, sequence len, embed dim)
      # Call _compute_contexts explicitly to get all_transition_contexts
      id_contexts, all_transition_contexts, _, _ = (
          self._trajectory_embedder._compute_contexts(
              [state[-1].trajectory for state in states]))
      if self._trajectory_embedder._use_ids:
        id_contexts = id_contexts + 0.1 * torch.randn_like(id_contexts)
        if len(id_contexts.shape) > 2:
          # If id_contexts is already computed for every time-step
          trajectory_embed = id_contexts
        else:
          trajectory_embed = id_contexts.unsqueeze(1).expand(
              -1, obs_embed.shape[1], -1)
      else:
        # NOTE: Regular 2-policy DREAM no longer runs the same.
        # To make it the same as before, we need to set trajectory_embed
        # to transition_context and tile it.
        trajectory_embed = all_transition_contexts
  ...
  def aux_loss(self, experiences):
    # Compute the auxiliary loss on the full trajectory.
    _, aux_losses = self._trajectory_embedder(
        [exp[0].trajectory for exp in experiences])
    return aux_losses

  @classmethod
  def from_config(cls, config, env, has_instructions=True, mode=None):
    ...
    if mode == "init-pos" or mode == "curr-pos" or mode == "valid-indices":
      id_embedder = SimpleGridStateEmbedder(
          env.observation_space["privileged_info"],
          config.get("transition_embedder").get("embed_dim"),
      )
    elif mode in ("static-demo", "dynamic-demo"):
      id_embedder = StatesGeneralEmbedder(
          priv_info_space=env.observation_space["privileged_info"].high,
          embed_dim=config.get("transition_embedder").get("embed_dim"),
          obs_len=len(env.observation_space["observation"].high),
          state_embedder=state_embedder,
      )
    elif mode == "optimal-act":
      id_embedder = OptimalActionEmbedder(
          priv_info_space=env.observation_space["privileged_info"].high,
          embed_dim=config.get("transition_embedder").get("embed_dim"),
          obs_len=len(env.observation_space["observation"].high),
    ...
    if config.get("trajectory_embedder").get("type") == "ours":
      trajectory_embedder = TrajectoryEmbedder(
          ...
          transition_embedder.embed_dim, id_embedder_mode=mode)
    else:
    ...
    return cls(trajectory_embedder, obs_embedder, instruction_embedder,
               ...
               config.get("use_trajectory_embed", True))
...
class StatesGeneralEmbedder(Embedder):
  def __init__(
      self,
      priv_info_space,
      embed_dim,
      obs_len: int,
      state_embedder: SimpleGridStateEmbedder,
  ):
    """
    Embedder when privileged info is a demo or the initial position.

    Args:
      priv_info_space (np.array): discrete max limits for each dimension of the state (expects min is 0).
      obs_len (int): length of observed states
      state_embedder (SimpleGridStateEmbedder): state embedder
    """
    super().__init__(embed_dim)
    self._obs_len = obs_len
    self._state_embedder = state_embedder
    input_dim = int(state_embedder.embed_dim * len(priv_info_space) / obs_len)
    self._fc_layer = nn.Linear(input_dim, embed_dim)

  def forward(self, priv_info):
    embeds = []
    for i in range(0, priv_info.shape[1], self._obs_len):
      start = i
      stop = i + self._obs_len
      state = priv_info[:, start:stop]
      embeds.append(self._state_embedder(tuple(state)))
...
class OptimalActionEmbedder(Embedder):
  def __init__(
      self,
      priv_info_space,
      embed_dim,
    ...
    Args:
      priv_info_space (np.array): discrete max limits for each dimension of the state (expects min is 0).
      obs_len (int): length of observed states
      action_embedder (FixedVocabEmbedder): action embedder
    """
    super().__init__(embed_dim)
    self._obs_len = obs_len
    self._action_embedder = action_embedder
    self._fc_layer = nn.Linear(self._action_embedder.embed_dim, embed_dim)

  def forward(self, priv_info):
    action_embedding = self._action_embedder._embedder(priv_info)
    x = self._fc_layer(action_embedding)
    return x
