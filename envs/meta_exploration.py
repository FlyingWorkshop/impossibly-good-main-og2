...
class InstructionWrapper(abc.ABC, gym.Wrapper):
  ...
  def __init__(self, env, exploration_trajectory=None, seed=None,
    ...
    if "privileged_info" in env.observation_space.spaces:
        observation_space.update({
          "privileged_info": env.observation_space["privileged_info"]
        })
...
class MetaExplorationEnv(abc.ABC, gym.Env):
  ...
  def __init__(self, env_id=0, wrapper=None):
    self._env_id = env_id
    self._wrapper = wrapper or (lambda state: torch.tensor(state))

    self._seed = self._env_id

  @classmethod
  def create_env_id(cls, seed, test=False):
  ...
  #       observations.
  #     mode (str): specifies what kind of privileged info is stored in the observation space (e.g., "demo", "env_id", "optimal")
  ...
  #   env_id = cls.create_env_id(seed, test)
  #   if mode:
  #     return cls(env_id, wrapper, mode=mode)
  #   else:
  ...
  def step(self, action):
    ...
    state, reward, done, _, info = self._step(action)
    if "privileged_info" in self.observation_space.spaces:
        state = MetaExplorationState(
            self._wrapper(state), reward, action, self._env_id, self._privileged_info)
    else:
      state = MetaExplorationState(
          self._wrapper(state), reward, action, self._env_id, None)
  ...
  def reset(self):
    # TODO: change seeding
    self._env_id = self.create_env_id(seed=0)
    state = self._reset()
    if "privileged_info" in self.observation_space.spaces:
      state = MetaExplorationState(
          self._wrapper(state), 0, None, self._env_id, self._privileged_info)
    else:
      # TODO: maybe need to remove step from state
      state = state[0]
      state = MetaExplorationState(
          self._wrapper(state), 0, None, self._env_id, None)
    return state
