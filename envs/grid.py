...
class GridEnv(meta_exploration.MetaExplorationEnv, abc.ABC):
  ...
  def render(self, mode="human"):
    ...
    image.write_text("Current state: {}".format(self._gen_obs()['observation']))
...
class GridRender(object):
  ...
  def draw_rectangle(self, position, size, color):
    ...
    """
    start = position * self._PIXELS_PER_GRID + (0.5 - size / 2) * np.array([self._PIXELS_PER_GRID, self._PIXELS_PER_GRID])
    end = position * self._PIXELS_PER_GRID + (0.5 + size / 2) * np.array([self._PIXELS_PER_GRID, self._PIXELS_PER_GRID])
